...
def payment_page(request, product_id):
    ...
    total = 0
    for info in dataC:
        total = total + round(float(info.price),2)


        # Calculate monthly
    mTotal = 0
    mData = Order.objects.filter(user = request.user).exclude(isRecurring=False)
    for info2 in mData:
        mTotal = mTotal + round(float(info2.price), 2)


        # Calculate once off
    oTotal = 0
    oData = Order.objects.filter(user = request.user).exclude(isRecurring=False)
    for oInfo2 in oData:
        oTotal = oTotal + round(float(oInfo2.price), 2)
